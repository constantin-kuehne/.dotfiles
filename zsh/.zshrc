...
plugins=(git docker conda-zsh-completion macos)
...
export PATH="$PATH:/Users/constantinkuehne/.lmstudio/bin"
# End of LM Studio CLI section

# Always use tmux's GPG_TTY if inside tmux
if [ -n "$TMUX" ]; then
    export GPG_TTY=$(tmux show-environment -g GPG_TTY 2>/dev/null | cut -d= -f2)
else
    export GPG_TTY=$(tty)
fi
...
gemini() {
    unset -f gemini
    export GEMINI_API_KEY=$(pass gemini-api)
    gemini
}

eval "$(starship init zsh)"
# transient prompt is the feature that collapses the prompt to one line after executing a command
# I use the brew package zsh-transient-prompt for this
